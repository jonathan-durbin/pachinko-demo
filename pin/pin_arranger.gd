...
@export var spacing_between_pins: float = 65.0
...
func arrange() -> void:
	...
	var max_x: float = screen_rect.size.x - pin_radius
	var max_y: float = screen_rect.size.y - end_y_from_bottom
	...
	while y <= max_y:
		...
		while x <= max_x:
			var p := pin.instantiate()
			add_child(p)
			p.owner = self.owner
			# Assuming that the scene assigned to pin has a position variable...
			p.position = Vector2(x, y)

			x += spacing_between_pins

		row += 1
		y += spacing_between_pins
